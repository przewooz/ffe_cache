...
uint64_t *Chromosome::piCachingFitness = nullptr;
uint64_t *Chromosome::piCachingPopulation = nullptr;
...
bool Chromosome::hasSeen() const {
	...
	if (it != cache.end())
	{
		if (Chromosome::piCachingFitness != nullptr)  (*Chromosome::piCachingFitness)++;
		return true;
	}//if (it != cache.end())
    return false;
}
...
bool Chromosome::tryFlipping(int index) {
    ...
    if (!twoEdge && getFitness() <= oldF || twoEdge && getFitness() - EPSILON <= oldF) {
        flip(index);
        evaluated = true;
        fitness = oldF;
