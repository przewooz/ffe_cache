...
void DSMGA2::comparativeMixing(int chIndex) {
	int r = myRand.uniformInt(0, ell - 1);

	Chromosome ch(ell);
	ch = population[chIndex];

	int patternIndex;

	do {
		patternIndex = myRand.uniformInt(0, nCurrent - 1);
	} while (patternIndex == chIndex);

	Chromosome& pattern = population[patternIndex];

	list<int>& candidateMask = masks[r];
	list<int> mask;

	ch.flip(candidateMask.front());
	mask.push_back(candidateMask.front());

	int value;

	for (list<int>::iterator it = ++candidateMask.begin(); it != candidateMask.end(); it++) {
		if (pattern.getVal(candidateMask.front()) == pattern.getVal(*it)) {
			value = ch.getVal(candidateMask.front());
		} else {
			value = 1 - ch.getVal(candidateMask.front());
		}

		if (value != ch.getVal(*it)) {
			mask.push_back(*it);
		}
	}

	size_t size = findSize(population[chIndex], mask);
	if (size > (size_t)ell / 2)
		size = ell / 2;
	...
	bool taken = restrictedMixing(population[chIndex], mask);
	...
	if (taken) {
		...
		for (int i = 0; i<nCurrent; ++i) {
			...
			if (EQ)
				backMixingE(population[chIndex], mask, population[orderN[i]]);
			else
				backMixing(population[chIndex], mask, population[orderN[i]]);
		}
	}
}
...
void DSMGA2::mixing() {
    ...
    for (int k=0; k<repeat; ++k) {
        ...
        genOrderN();
        for (int i=0; i<nCurrent; ++i) {
			if (useComparativeMixing) {
				comparativeMixing(orderN[i]);//asdasd
			} else {
...
inline bool DSMGA2::isInP(const Chromosome& ch) const {
	...
	b_result = (it != pHash.end());

	if (b_result == true)
	{
		if (Chromosome::piCachingPopulation != nullptr)  (*Chromosome::piCachingPopulation)++;
	}//if ((b_result == true)
