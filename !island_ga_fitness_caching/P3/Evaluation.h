...
class CBinaryCoding;
template <class TFenotype> class CEvaluation;
...
public: 
	...
	float  fEvaluate(Evaluator& evaluator);
...
class Evaluator {
 public:
  ...
  float virtual max_fitness(const float max_fitness_config) { return max_fitness_config; };
  size_t virtual length(const size_t length_config) { return length_config; };
};
...
// Discretization of the common Rosenbrock function into grey coded bits.
class Rosenbrock : public Evaluator {
public:
	Rosenbrock(Configuration& config, int run_number);
	float evaluate(const vector<bool> & solution) override;
	create_evaluator(Rosenbrock);

private:
	int precision;
	// Tools used to convert bits to function values.
	BinaryToFloat converter;
	unordered_map<pair<float, float>, float, pair_hash> function;
	float worst;
};
...
class Injected : public Evaluator {
public:
	Injected(Configuration& config, int run_number);
	float evaluate(const vector<bool> & solution) override;
	float max_fitness(const float max_fitness_config) override;
	size_t length(const size_t length_config) override;
	create_evaluator(Injected);

private:
	CEvaluation<CBinaryCoding>* injected_evaluation;
	CBinaryCoding individual;
};
...
static std::unordered_map<string, pointer> lookup( {
	...
	{ "Rosenbrock", Rosenbrock::create },
    { "External", External::create },
	{ "Injected", Injected::create },
});
}
