...
Population::Population(Configuration& config, Evaluator& evaluator) {
  length = evaluator.length(config.get<int>("length"));
...
void Population::rebuild_tree(Random& rand) {
  ...
  for (size_t index = length; index < clusters.size(); index++) {
    ...
    if (round_float(distances[first][second], precision) == 0) {
...
bool Population::donate(CLtgaIndividual & solution,
			CLtgaIndividual & source, const vector<int> & cluster,
                        Evaluator& evaluator) {
	...
	f_fitness_old = solution.fEvaluate(evaluator);
  ...
  for (const auto& index : cluster) {
    changed |= (solution.v_genotype[index] != source.v_genotype[index]);
    // uses the "source" to store the original value of "solution"
    vector<bool>::swap(solution.v_genotype[index], source.v_genotype[index]);
  }
  ...
  if (changed) {
    ...
    //float new_fitness = evaluator.evaluate(solution);
	  //check if fitness was not in the pop
	  if (pv_fitnesses != nullptr)
	  {
		  unordered_map<vector<bool>, double>::iterator it = pv_fitnesses->find(solution.v_genotype);
		  if (it != pv_fitnesses->end())
		  {
			  if (CLtgaIndividual::piCachingPopulation != nullptr)  (*CLtgaIndividual::piCachingPopulation)++;
			  
			  solution.b_rated = true;
			  solution.f_fitness = (*pv_fitnesses)[solution.v_genotype];
		  }//if (it != cache.end())
	  }//if (pv_fitnesses != nullptr)

	  float new_fitness = solution.fEvaluate(evaluator);
    // NOTE: My previous work used strict improvement
    if (f_fitness_old <= new_fitness) {
      // improvement made, keep change to solution
      //fitness = new_fitness;
      // copy pattern back into the source, leave solution changed
      for (const auto& index : cluster) {
        source.v_genotype[index] = solution.v_genotype[index];
      }
    } else {
...
void Population::improve(Random& rand, CLtgaIndividual & solution, Evaluator& evaluator) {
  ...
  for (auto& cluster_index : cluster_ordering) {
    ...
    while (unused >= 0 && !different) {
      ...
      different = donate(solution, solutions[working], cluster, evaluator);
