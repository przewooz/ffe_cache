...
class Pyramid : public Optimizer {
 public:
  ...
  bool feedback(CLtgaIndividual & solution, float & fitness);
  // Handles adding a solution to the population.
  // Returns true if it was unique and therefore added.
  bool add_unique(CLtgaIndividual & solution, size_t level);
  // Iteratively improves the solution using the pyramid of populations
  // leverages the Population class extensively
  bool climb(CLtgaIndividual & solution);
  bool climb(CLtgaIndividual & solution, vector<size_t> & addedToLevels);
  bool climb(CLtgaIndividual & solution, size_t level, bool hillClimber, bool climbOnlyIfUnique);
