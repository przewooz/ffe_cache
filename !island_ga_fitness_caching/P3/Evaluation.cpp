...
bool CACHE_LTGA_P3 = true;
unordered_map<unsigned long, double> cache;

uint64_t *CLtgaIndividual::piCachingFitness = nullptr;
uint64_t *CLtgaIndividual::piCachingPopulation = nullptr;


static uint64_t *piCachingFitness;
static uint64_t *piCachingPopulation;
static std::unordered_map<vector<bool>, double> m_cache;


float  CLtgaIndividual::fEvaluate(Evaluator& evaluator)
{
	if (b_rated)  return(f_fitness);

	f_fitness = evaluator.evaluate(v_genotype);
	b_rated = true;
...
float DeceptiveTrap::evaluate(const vector<bool> & solution) {
  ...
  // Convert to percentage of total
  return round_float(float(total) / solution.size(), precision);
}
...
float DeceptiveStepTrap::evaluate(const vector<bool> & solution) {
  ...
  return round_float(fitness, precision);
}
...
float NearestNeighborNK::solve(vector<bool>& solution, bool maximize) {
    ...
    for (size_t left = 0; left < numbers; left++) {
      for (size_t right = 0; right < numbers; right++) {
        ...
        for (size_t middle = 0; middle < numbers; middle++) {
          ...
          if ((maximize && utility[left][right] < current)
              || (!maximize && utility[left][right] > current)) {
  ...
  for (size_t left = 0; left < numbers; left++) {
    for (size_t right = 0; right < numbers; right++) {
      ...
      if ((maximize && fitness < current)
          || (!maximize && fitness > current)) {
        fitness = current;
        best_left = left;
        best_right = right;
      }
...
float LeadingOnes::evaluate(const vector<bool> & solution) {
  for (size_t i = 0; i < solution.size(); i++) {
    if (!solution[i]) {
      return round_float(float(i) / solution.size(), precision);
    }
  }
...
MAXSAT::MAXSAT(Configuration& config, int run_number) {
  ...
  precision = config.get<int>("precision");
  clauses.resize(
	  round_float(config.get<float>("clause_ratio") * length, precision));
...
float MAXSAT::evaluate(const vector<bool> & solution) {
  ...
  // Convert to percentage of total
  return round_float(float(total) / clauses.size(), precision);
}
...
float IsingSpinGlass::evaluate(const vector<bool>& solution) {
  ...
  // Convert to percentage of total
  return round_float(1 - (energy - min_energy) / span, precision);
}
...
float Rastrigin::evaluate(const vector<bool>& solution) {
  ...
  // Convert to percentage of total
  total /= (n * worst);
  return round_float(1 - total, precision);
}

// Sets up a table of fitness function values based on possible gray
// value settings
Rosenbrock::Rosenbrock(Configuration& config, int run_number)
	: precision(config.get<int>("precision")),
	converter(
		BinaryToFloat(config.get<int>("bits_per_float"), -2.048, 2.048,
			precision)) {
	worst = 0;

	for (const auto& x : converter.possible()) {
		for (const auto& y : converter.possible()) {
			float value = 100 * (y - x * x) * (y - x * x) + (1 - x) * (1 - x);
			function[pair<float, float>(x, y)] = value;
			if (worst < value) {
				worst = value;
			}
		}
...
// Converts each group of bits into a grey value, then look up the fitness
float Rosenbrock::evaluate(const vector<bool>& solution) {
	auto it_x = solution.begin();
	float x, y, total = 0;
	int n = 0;
	bool end = false;
	...
	// Use the converter to turn bits in the solution into float values
	while (!end) {
		x = converter.convert(it_x);

		auto it_y(it_x);
		y = converter.convert(it_y);

		total += function[pair<float, float>(x, y)];
		
		n++;
		end = it_y == solution.end();
	}
...
Injected::Injected(Configuration& config, int run_number)
	: individual(config.get_injected_evaluation()->iGetNumberOfElements())
{
	injected_evaluation = config.get_injected_evaluation();
}


bool hasSeen(const vector<bool>& solution)
{
	unordered_map<vector<bool>, double>::iterator it = m_cache.find(solution);
	if (it != m_cache.end())
	{
		if (CLtgaIndividual::piCachingFitness != nullptr)  (*CLtgaIndividual::piCachingFitness)++;
		return true;
	}//if (it != cache.end())
	return false;
}//bool hasSeen(unsigned long ulKey)*/



float Injected::evaluate(const vector<bool>& solution)
{
	for (size_t i = 0; i < solution.size(); i++) {
		individual.piGetBits()[i] = (int32_t)solution.at(i);
	}
	...
	if (CACHE_LTGA_P3)
	{
		if (hasSeen(solution)) {
			...
			return m_cache[solution];
		}
	}//if (CACHE_LTGA_P3)

	double  d_fitness;
	d_fitness = injected_evaluation->dEvaluate(&individual);
	//return injected_evaluation->dEvaluate(&individual);

	if (CACHE_LTGA_P3)
		m_cache[solution] = d_fitness;

	return(d_fitness);
}

float Injected::max_fitness(const float max_fitness_config) {
	return (double)injected_evaluation->dGetMaxValue();
}

size_t Injected::length(const size_t length_config) {
	return (size_t)injected_evaluation->iGetNumberOfElements();
}