...
void CSystem::v_register_commands()
{
	...
	v_register_command(new CGenerateRandomMaxSat(&c_log, &f_result_summary));
	v_register_command(new CPlantedMaxSat2ProblemGeneration(&c_log, &f_result_summary));
	v_register_command(new CAvisSubsetSumKnapsackProblemGenerator(&c_log, &f_result_summary));
	v_register_command(new CInjectedPopulationOptimizerRepeatabilityCheckingCommand(&f_settings, &c_log, &f_result_summary));
	v_register_command(new CChangeDetectionCommand(&f_settings, &c_log, i_random_seed, &f_result_summary));
	v_register_command(new CReportDeceptive(&f_settings));
	v_register_command(new CRandomSet(&f_settings, &c_log));
}//void CSystem::v_register_commands()
...
CError CSystem::e_read_arguments_from_file(int iArgCount, char **ppcArgValues)
{
	...
	else
	{
		...
		s_log_file = "log.txt";
		s_file_name = "C:\\Projekty\\projekty_pwr\\island_ga\\x64\\Release\\settings_3lo.txt";
