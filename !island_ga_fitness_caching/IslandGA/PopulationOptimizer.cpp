	...
	if (!c_error)
	{
		...
		if (!c_error)
		{
			...
			else
			{
				...
				if (!c_error)
				{
					pc_generation = GenerationUtils::pcGetGeneration(pc_problem, pl_gene_patterns, psSettings, &c_error);
				}//if (!c_error)
...
template class CPopulationOptimizer<CRealCoding, CRealCoding>;