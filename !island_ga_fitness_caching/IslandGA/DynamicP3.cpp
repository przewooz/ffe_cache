...
CDynamicP3::CDynamicP3(CProblem<CBinaryCoding, CBinaryCoding> *pcProblem, CLog *pcLog, uint32_t iRandomSeed)
...
void CDynamicP3::vChangeState(time_t tStartTime, uint64_t iStartFFE)
{
	...
	for (size_t i = 0; i < pc_p3->pc_p3->pops.size(); i++)
	{
		...
		{
			pc_p3->pc_recorder->evaluate(pc_p3->pc_p3->pops.at(i).solutions.at(j).v_genotype);
		}//for (size_t j = 0; j < pc_p3->pc_p3->pops.at(i).solutions.size(); j++)
	}//for (size_t i = 0; i < pc_p3->pc_p3->pops.size(); i++)
...
{
	pc_static_optimizer = new CP3(pc_problem, pc_log, i_random_seed);
