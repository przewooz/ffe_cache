...
CDynamicOptimizer<TGenotype, TFenotype>::CDynamicOptimizer(CProblem<TGenotype, TFenotype> *pcProblem, CLog *pcLog, uint32_t iRandomSeed)
{
...
template <class TGenotype, class TFenotype>
CDynamicOptimizer<TGenotype, TFenotype> * DynamicOptimizer::pcGetDynamicOptimizer(CProblem<TGenotype, TFenotype> *pcProblem, CLog *pcLog, uint32_t iRandomSeed, istream *psSettings, CError *pcError, bool bIsObligatory)
{
	...
	if (!*pcError && p_type.bHasValue())
	{
		switch (e_type)
		{
			...
			{
				pc_dynamic_optimizer = (CDynamicOptimizer<TGenotype, TFenotype>*)new CDynamicP3((CProblem<CBinaryCoding, CBinaryCoding>*)pcProblem, pcLog, iRandomSeed);
				break;
			}//case DYNAMIC_OPTIMIZER_P3
			case DYNAMIC_OPTIMIZER_LTGA:
			{
				pc_dynamic_optimizer = (CDynamicOptimizer<TGenotype, TFenotype>*)new CDynamicLTGA((CProblem<CBinaryCoding, CBinaryCoding>*)pcProblem, pcLog, iRandomSeed);
				break;
			}//case DYNAMIC_OPTIMIZER_LTGA
			case DYNAMIC_OPTIMIZER_DSMGA2:
			{
				pc_dynamic_optimizer = (CDynamicOptimizer<TGenotype, TFenotype>*)new CDynamicDSMGA2((CProblem<CBinaryCoding, CBinaryCoding>*)pcProblem, pcLog, iRandomSeed);
				break;
			}//case DYNAMIC_OPTIMIZER_DSMGA2
			default:
			{
...
template class CDynamicOptimizer<CRealCoding, CRealCoding>;


template CDynamicOptimizer<CBinaryCoding, CBinaryCoding> * DynamicOptimizer::pcGetDynamicOptimizer(CProblem<CBinaryCoding, CBinaryCoding>*, CLog*, uint32_t, istream*, CError*, bool);
template CDynamicOptimizer<CRealCoding, CRealCoding> * DynamicOptimizer::pcGetDynamicOptimizer(CProblem<CRealCoding, CRealCoding>*, CLog*, uint32_t, istream*, CError*, bool);