...
{
	CLtgaIndividual::piCachingFitness = &i_caching_fitness;
	CLtgaIndividual::piCachingPopulation = &i_caching_population;
...
bool CP3::bClimb(CBinaryCoding *pcGenotype, uint32_t iIterationNumber, time_t tStartTime)
{
	...
	c_new_ind.v_genotype.reserve(pcGenotype->iGetNumberOfBits());

	for (uint16_t i = 0; i < pcGenotype->iGetNumberOfBits(); i++)
	{
		c_new_ind.v_genotype.push_back(*(pcGenotype->piGetBits() + i) == 1);
	}//for (uint16_t i = 0; i < pcGenotype->iGetNumberOfBits(); i++)

	//float f_p3_individual_fitness_value = pc_recorder->evaluate(c_new_ind.v_genotype);

	vector<size_t> v_added_to_levels;

	if (pc_p3->climb(c_new_ind, v_added_to_levels))
	{
